...
import { catalogOrigin } from "../app/domain/catalog-contract.ts";
import { isTeachingGlyph } from "./lib/teaching-glyphs.mjs";
...
const webFontStyles = join(projectRoot, "app/generated-font.css");
const miniFontStyles = join(projectRoot, "wechat-miniprogram/miniprogram/generated-font.wxss");
...
async function main() {
  ...
  const fontFace = (url) => `/* Generated by scripts/build-kai-subset.mjs. Do not edit. */
@font-face {
  font-family: "Knowing Kai";
...
`;
  await Promise.all([
    writeFile(webFontStyles, fontFace(`/fonts/lxgw-wenkai-subset.woff2?h=${contentHash}`)),
    writeFile(
      miniFontStyles,
      fontFace(`${catalogOrigin}/api/mini-font/v1/lxgw-wenkai.woff2?h=${contentHash}`),
    ),
  ]);
  console.log(
    `wrote public/fonts/lxgw-wenkai-subset.woff2 — ${(bytes.length / 1024).toFixed(0)} KB, sha256 ${contentHash}`,
  );
}
