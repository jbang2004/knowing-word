...
这是与 Web 版同仓库维护的微信原生 TypeScript 客户端。页面使用 WXML/WXSS 重建，课程完整内容从现有 Sites API 按课加载并缓存在本机，进度、学习事件和录音继续写入现有 D1/R2。430 条正式旁白在构建时转为 M4A，并作为不可变 Site 资源随同版本发布，避免依赖平台内部 R2 桶名。
