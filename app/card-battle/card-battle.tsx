...
export default function CardBattle({ questions, lessonId, lessonTitle, lessons }: {
  ...
  return <main className={`cb-game ${paused ? "cb-paused" : ""} ${impact ? result?.correct ? "cb-hit" : "cb-hurt" : ""}`} data-phase={state.phase}>
    <div className="cb-world" aria-hidden="true" />
    ...
    {overlay && <div className="cb-overlay"><section ref={modalRef} className="cb-modal" role="dialog" aria-modal="true" aria-labelledby="cb-modal-title">
      {!isEnd && <button className="cb-modal-close" aria-label="关闭" onClick={() => { setPaused(false); setHelp(false); setLessonMenu(false); }}><X /></button>}
      {help ? <><span className="cb-modal-emblem"><BookOpen /></span><span className="cb-eyebrow">唤字师手札</span><h2 id="cb-modal-title">让知识成为你的法术</h2><div className="cb-rules"><p><b>01 · 观察意图</b>守关者会预告伤害。蓄力时放心进攻，重击前用结界保护自己。</p><p><b>02 · 选牌答题</b>选一张牌，再回答课内原题。答对施法；答错不施法，反击额外增加 6 点伤害。</p><p><b>03 · 搭配取胜</b>连续答对每次增加 4 点伤害，最多 12 点。雷霆与治疗有冷却，合理搭配五张牌。</p><p><b>04 · 穿越秘境</b>击败三位守关者即获胜，每过一关恢复 24 生命。战斗不会替代课程过关记录。</p></div><button className="cb-primary" onClick={() => setHelp(false)}>我准备好了 <Sparkles size={16} /></button></> : lessonMenu ? <><span className="cb-eyebrow">选择你的知识法典</span><h2 id="cb-modal-title">从哪一课开始冒险？</h2><p>切换课文会开启一场新冒险。</p><div className="cb-lesson-list">{lessons.map(lesson => <Link aria-current={lesson.id === lessonId ? "page" : undefined} key={lesson.id} href={`/card-battle?lesson=${lesson.id}`} onClick={() => setLessonMenu(false)}><span>{String(lesson.position).padStart(2, "0")}</span>{lesson.title}{lesson.id === lessonId && <Check size={16} />}</Link>)}</div></> : paused ? <><span className="cb-modal-emblem"><Pause /></span><span className="cb-eyebrow">秘境正在等你</span><h2 id="cb-modal-title">稍作休息</h2><p>调整呼吸，下一道法术就在你的手中。</p><button className="cb-primary" onClick={() => setPaused(false)}><Play size={17} />继续冒险</button><button className="cb-secondary" onClick={restart}><RotateCcw size={16} />重新开始</button><Link className="cb-secondary" href="/practice">返回练习</Link></> : <><span className="cb-modal-emblem">{state.phase === "defeat" ? <Heart /> : <Crown />}</span><span className="cb-eyebrow">{state.phase === "victory" ? "三重秘境 · 全部净化" : state.phase === "defeat" ? "每次尝试，都有收获" : "守关者已被净化"}</span><h2 id="cb-modal-title">{state.phase === "victory" ? "字灵觉醒，秘境重生" : state.phase === "defeat" ? "星火未熄，再试一次" : `${bossNames[state.bossIndex]}已苏醒`}</h2><p>本次答对 {state.correctAnswers} / {state.answeredQuestions} 题，唤醒 {state.correctAnswers} 道法术。</p>{question && result && <div className="cb-end-explanation"><strong>{result.correct ? "最后一道法术生效" : "记住这一题，下次一定行"}</strong><p>{question.explanation}</p></div>}<button className="cb-primary" onClick={state.phase === "boss-defeated" ? next : restart}>{state.phase === "boss-defeated" ? "恢复 24 生命 · 前往下一秘境" : "再来一场"}<ArrowRight size={16} /></button><Link href="/practice" className="cb-secondary">返回练习</Link></>}
    </section></div>}
  </main>;
}
